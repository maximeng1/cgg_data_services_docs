...
You can trigger 3 different types of email. 
Please note that API calls will trigger specific journeys but all rules to be applied to the time of send, number of emails to be sent and any A/B tests are set up in the UI. 
...
### Abandoned Checkout email to be sent to customers to resume application (if dropped off after step one of the application).
### Required Query Parameters for Initial Event
